...
router.post('/adminlogin',(req,res) =>{
    ...
    con.query(sql, [req.body.email], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            bcrypt.compare(req.body.password, hashPassword, (err, isMatch) => {
                if (err || !isMatch) {
                    return res.json({ loginStatus: false, Error: "wrong email or password" });
                }
                const token = jwt.sign(
                    { role: "admin", email: result[0].email, id: result[0].id },
                    "myemployee_secret_key",
                    { expiresIn: "1d" }
                );
  ...
  router.post("/add_admin", (req, res) => {
    ...
    bcrypt.hash(password, 10, (err, hashPassword) => {
        if (err) {
            return res.json({ loginStatus: false, Error: "Error in hashing password" });
        }

        const sql = "INSERT INTO admin (email, password) VALUES (?, ?)";
        con.query(sql, [email, hashPassword], (err, result) => {
            if (err) {
